    ...
    <div className="workspacePage">
      ...
      <div className="adminSplit">
        ...
        {booth && <div className="adminDetail">
          ...
          {booth.widthMm && booth.depthMm && <div className="adminPreviews">{booth.thumbnailUrl && <div><span>THUMBNAIL</span><img className="catalogPhotoPreview" src={booth.thumbnailUrl} alt={`Náhled ${booth.name}`} /></div>}<div><span>2D NÁHLED</span><div className="adminPlanPreview"><BoothCadPlanView asset={getMasterReferenceModel(booth.assets)} boothAsset={booth.boothAsset} footprintWidthMm={booth.widthMm} footprintDepthMm={booth.depthMm} visible selected={false} /></div></div><div><span>3D NÁHLED</span><BoothCadViewer asset={getMasterReferenceModel(booth.assets)} footprintWidthMm={booth.widthMm} footprintDepthMm={booth.depthMm} components={[]} defaultViews={booth.defaultViews} nominalDimensions={booth.nominalDimensions} /></div></div>}
        </div>}
      </div>
    </div>
  );
