...
export async function handleEmailHistoryDelete(
  request: NextRequest,
  repositoryFactory: () => EmailHistoryRepository = defaultRepositoryFactory,
): Promise<NextResponse> {
  if (!(await isSessionRequestAuthorized(request))) return NextResponse.json({ error: "Pro mazání historie e-mailů je vyžadováno přihlášení." }, { status: 401 });
  let body: DeleteBody;
  ...
  try {
    ...
    return NextResponse.json({ error: "Záznam historie se nepodařilo smazat." }, { status: 502 });
  }
}

export async function POST(request: NextRequest) {
  return handleEmailHistoryDelete(request);
}
